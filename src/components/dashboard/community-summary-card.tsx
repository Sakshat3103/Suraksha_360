...
export function CommunitySummaryCard({
  ...
  React.useEffect(() => {
    ...
    if (!topCategory) {
      ...
      setSummary({ text: "No reports filed nearby in the last 7 days.", riskLevel: "Low" });
      return;
    }
      ...
      <CardContent className="flex flex-col gap-3">
        {!summary ? (
          <span className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <Loader2 className="size-3.5 animate-spin" /> Analyzing nearby reports…
          </span>
        ) : (
          <>
            ...
            <div className={cn("flex w-fit items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold", toneClass)}>
              <AlertTriangle className="size-3.5" /> Risk Level: {summary.riskLevel}
            </div>
          </>
        )}
      </CardContent>
